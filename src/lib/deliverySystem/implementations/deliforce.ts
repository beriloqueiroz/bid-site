...
async function sendTask(
    { name, email, address, orderNumber, value, description, startDate, endDate, phone, complement, reference, account }: SendTask
...
function getDescStatus(taskStatus: string, notes?: string, imageArry?: string[]): TaskStatus {
    if (notes)
        return TaskStatus.DELIVERY_NOTE_ADDED;

    if (imageArry && imageArry?.length > 0)
        return TaskStatus.DELIVERY_PROTOCOL_ADDED;
